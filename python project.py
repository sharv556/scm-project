...
import datetime                    #Deltatime library, to get Real Date infoRSation.
import os                          #OS (Operating system) , To provide cross-platfoRS compatibility
...
def def_full_file_reader():                                                                         #mustafa
    ...
    i = 0
    while i <= (len(list_foods) - 1): #Enumarte through food list to filter out prices and setup print FoRSatting by replacing spaces with count difference of string length and align Prices to the most left of the teRSinal
...
def def_food_drink_order():
    while True:
            print("*" * 26 + "SQUARE ONE FOOD MENU" + "*" * 26)
            print(" |NO| |FOOD NAME|         |PRICE|   |  |NO| |DRINK NAME|        |PRICE|")

            i = 0
            while i < len(list_foods) or i < len(list_drinks):
                var_space = 1
                if i <= 8:                      # To fix up to space indention in console or teRSinal by applying detection rule to figure out spacing for TWO DIGITS numbers
                    var_space = 2
    ...
    while True:
        ...
        while i < len(list_item_order): #Enumarating order array items and summing up its prices * quantities
            if(list_item_order[i] != 0):
                if (i >= 0) and (i < 40):
                    report_new += "\n" + " " * 17 + str(list_foods[i]) + "  x  " + str(list_item_order[i]) # string appending the foRSated food name and foRSated order structure from quantity and final price
